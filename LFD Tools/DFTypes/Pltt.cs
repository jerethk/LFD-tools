...
public class Pltt
{
    ...
    public static Pltt GetBrfJanPltt()
    {
        var pltt = new Pltt();
        pltt.Name = "BRF-JAN";
        pltt.FirstColour = 0;
        ...
        var data = BrfJan.Split();
        for (int c = 0; c < 256; c++)
        {
            pltt.Colours[c].R = Convert.ToByte(data[c * 3]);
            pltt.Colours[c].G = Convert.ToByte(data[c * 3 + 1]);
            pltt.Colours[c].B = Convert.ToByte(data[c * 3 + 2]);
        }
